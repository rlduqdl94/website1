...
$sql = "UPDATE dbo.regist_table SET acc_name = '$acc_name', due_date  = '$due_date',regist_status1 = '$regist_status1', product_status = '$product_status', del_place = '$del_place' WHERE regist_no = '$where_data'";
sqlsrv_query($con,$sql);

$sql = "DELETE FROM dbo.regist_data WHERE regist_no = '$where_data'";
sqlsrv_query($con,$sql);

for ($i=0; $i < $product_count; $i++) {
  $sql = "INSERT INTO dbo.regist_data (regist_no,pro_name,pro_count,pro_price,pro_stand,pro_height,pro_text,pro_category,image_url) VALUES('$where_data','$name_arr[$i]','$count_arr[$i]','$price_arr[$i]','$stand_arr[$i]','$height_arr[$i]','$text_arr[$i]','$category_arr[$i]','$image_arr[$i]')";
  sqlsrv_query($con,$sql);
}
